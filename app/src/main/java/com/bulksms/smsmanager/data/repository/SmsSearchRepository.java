...
public class SmsSearchRepository {
    ...
    public Completable buildSearchIndex() {
        return Completable.fromAction(() -> {
            try {
                ...
                Log.e(TAG, "Failed to build search index", e);
                _errorState.postValue("Failed to build search index: " + e.getMessage());
    ...
    public Completable indexMessage(SmsEntity message) {
        return Completable.fromAction(() -> {
            try {
                ...
                Log.e(TAG, "Failed to index message", e);
                _errorState.postValue("Failed to index message: " + e.getMessage());
                throw new RuntimeException(e);
            }
        }).subscribeOn(Schedulers.io());
    }
    ...
    private SmsFtsEntity createFtsEntity(SmsEntity message) {
        try {
            ...
            // Note: SmsFtsEntity stores only phoneNumber and message
