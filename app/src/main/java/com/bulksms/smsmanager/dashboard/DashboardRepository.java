...
public class DashboardRepository {
    ...
    public DashboardRepository(Context context, FirebaseBillingManager billingManager) {
        this.context = context;
        this.billingManager = billingManager;
    }

    public void loadDashboardStats(Callback<DashboardStats> callback) {
        // Load SMS stats and billing info concurrently
        CompletableFuture<SmsStats> smsStatsFuture = loadSmsStats();
        CompletableFuture<SubscriptionInfo> billingFuture = loadBillingInfo();

        CompletableFuture.allOf(smsStatsFuture, billingFuture)
            .thenAccept(v -> {
                try {
                    SmsStats smsStats = smsStatsFuture.get();
                    SubscriptionInfo billingInfo = billingFuture.get();
                    DashboardStats stats = new DashboardStats(smsStats, billingInfo);
    ...
    private CompletableFuture<SubscriptionInfo> loadBillingInfo() {
        return billingManager.getCurrentSubscription()
            .exceptionally(throwable -> null); // Return null if billing fails
    }
