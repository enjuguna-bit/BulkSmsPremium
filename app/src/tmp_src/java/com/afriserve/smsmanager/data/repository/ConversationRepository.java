            ...
            try {
                ...
                for (java.util.Map.Entry<String, List<SmsEntity>> entry : messagesByPhone.entrySet()) {
                    ...
                    if (!messages.isEmpty()) {
                        ...
                        Log.d(TAG, "Created conversation for " + phoneNumber + " with " + messages.size() + " messages");
                    }
                }
                
                Log.d(TAG, "Created " + conversations.size() + " conversations total");
                
                // Insert all conversations
                if (!conversations.isEmpty()) {
                    conversationDao.insertConversations(conversations).blockingAwait();
                    Log.d(TAG, "Inserted " + conversations.size() + " conversations into database");
                }
