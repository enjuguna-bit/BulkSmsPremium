...
public class FirebaseBillingManager {
    ...
    // Plans -> Moved to SubscriptionPlans
    // definitions removed
                        ...
                        return updateUserPlan(userId, planId)
                                .thenCompose(aVoid -> {
                                    ...
                                    return recordBillingEvent(
                                            ...
                                            amount,
                                            paymentMethod,
                                            transactionId).thenApply(voidResult -> docRef);
                                });
                    })
                                ...
                                .addOnSuccessListener(aVoid -> {
                                    ...
                                    processRefund(document.getData())
                                            .thenCompose(refundResult -> {
                                                ...
                                                return updateUserPlan(userId, SubscriptionPlans.PLAN_FREE);
                                            })
                                            .thenCompose(aVoid2 -> {
        ...
        try {
            ...
            if (amount != null && startDate != null && endDate != null) {
                ...
                recordBillingEvent(
                        ...
                        (String) subscriptionData.get("transaction_id")).thenAccept(aVoid -> future.complete(null))
                        .exceptionally(throwable -> {
    ...
    public CompletableFuture<UsageStats> getUsageStats() {
        ...
        try {
            ...
            dailyQuery.thenCompose(dailySnapshot -> monthlyQuery.thenApply(monthlySnapshot -> {
                int dailyTotal = 0;
    ...
    private CompletableFuture<Boolean> isTransactionUsed(String transactionId) {
        ...
        firestore
                ...
                .get()
                .addOnSuccessListener(snapshot -> future.complete(snapshot.getDocuments().size() > 0))
                .addOnFailureListener(future::completeExceptionally);
    ...
    private SubscriptionInfo documentToSubscription(DocumentSnapshot document) {
        ...
        return new SubscriptionInfo(
                document.getId(),
                document.getString("plan_id") != null ? document.getString("plan_id") : SubscriptionPlans.PLAN_FREE,
    ...
    private CompletableFuture<SubscriptionInfo> createFreeSubscription() {
        ...
        try {
            ...
            SubscriptionPlan planDetails = getPlanDetails(SubscriptionPlans.PLAN_FREE);

            Map<String, Object> subscription = new HashMap<>();
            subscription.put("plan_id", SubscriptionPlans.PLAN_FREE);
            subscription.put("status", STATUS_ACTIVE);
            subscription.put("start_date", System.currentTimeMillis());
